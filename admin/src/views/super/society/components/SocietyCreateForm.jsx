...
const SocietyCreateForm = ({ onSubmit, onEditHandler }) => {
  ...
  const rawSubscriptionPlans = useSelector(
    (state) => state.customer.subscriptionPlans
  );
  const [subscriptionPlans, setSubscriptionPlans] = useState([]);

  useEffect(() => {
    if (!rawSubscriptionPlans || !Array.isArray(rawSubscriptionPlans)) {
      setSubscriptionPlans([]);
      return;
    }

    // Debugging
    console.log("Raw subscription plans:", rawSubscriptionPlans);
    
    // Try different approaches based on the data structure
    try {
      // Approach 1: Filter by stringified value
      const uniqueValues = [];
      const seenValues = new Set();
      
      rawSubscriptionPlans.forEach(plan => {
        // Generate a key based on what makes a plan "unique"
        // Adapt this to your actual data structure
        const planKey = typeof plan === 'object' ? 
          (plan.id || plan.value || JSON.stringify(plan)) : 
          String(plan);
        
        if (!seenValues.has(planKey)) {
          seenValues.add(planKey);
          uniqueValues.push(plan);
        }
      });
      
      console.log("Unique plans after filtering:", uniqueValues);
      setSubscriptionPlans(uniqueValues);
    } catch (error) {
      console.error("Error filtering subscription plans:", error);
      // Fallback to the original array if there's an error
      setSubscriptionPlans(rawSubscriptionPlans);
    }
