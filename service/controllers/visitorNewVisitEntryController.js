...
exports.createVisitorNewVisitEntryController = async (req, res) => {
  try {
    console.log("Full request body received:", req.body); // Debugging
    ...
    // Check if societyId is missing
    if (!societyId) {
      console.error("Missing societyId in request body:", req.body);
      return sendErrorResponse(res, "Missing societyId", 400);
    }
...
exports.getVisitorEntriesBySender = async (req, res) => {
  try {
    ...
    if (!senderId) {
      return sendErrorResponse(res, "Enter Sender Id", 400);
    }

    const senderIdInt = parseInt(senderId);

    if (isNaN(senderIdInt)) {
      return sendErrorResponse(res, "Invalid Sender Id", 400);
    }

    const visitorEntries = await Visitor_new_visitentry.findAll({
      where: { senderId: senderIdInt, isDeleted: false },
    });

    if (!visitorEntries || visitorEntries.length === 0) {
      return sendErrorResponse(res, "No visitor entries found for this sender", 404);
    }

    const visitorEntriesWithQR = await Promise.all(
      visitorEntries.map(async (visitorEntry) => {
        const qrData = {
          visit_entry_Id: visitorEntry.visit_entry_Id,
          societyId: visitorEntry.societyId,
          visit_name: visitorEntry.visit_name,
