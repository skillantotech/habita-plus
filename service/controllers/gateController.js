...
// Controller example
exports.createGates = async (req, res) => {
  const gates = req.body; // An array of gate objects
  try {
    const createdGates = await Gate.bulkCreate(gates);
    res.status(201).json(createdGates);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to create gates." });
  }
};
